...
// Auxílio Sleep – Scriptable

function iniciar() {
  let alert = new Alert();
  alert.title = "Auxílio Sleep";
  alert.message = "Digite sua key:";
  alert.addTextField("Key");
  alert.addAction("Entrar");
  alert.addCancelAction("Cancelar");

  alert.present().then(index => {
    let key = alert.textFieldValue(0);
    if (key === "key-789173900") {
      let web = new WebView();
      web.loadHTML(htmlPainel());
      web.present();
    } else {
      let error = new Alert();
      error.title = "Erro";
      error.message = "Key incorreta!";
      error.addAction("OK");
      error.present();
    }
  });
